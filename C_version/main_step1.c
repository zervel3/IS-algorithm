...
void Generate_uniform_Fstar_and_Gstar(char **argv) ;
...
int main(int agrs, char **argv){
	...
	Generate_uniform_Fstar_and_Gstar(argv) ; //Generate the Influence and susceptibility scores
											 // then use them to calculate F_star and G_star(the two variable appeared in equations 3 and 4 in main text)
	Init_Influence_and_Susceptibility() ;  //Before the iterative algorithm start, set the I^{(0)} and S^{(0)}
	Iterate_Influence_label(atoi(argv[3]),argv) ;  // Start the IS iterative algorithm
...
void Generate_uniform_Fstar_and_Gstar(char **argv) 
{
	...
	while(i<N)
	{
		...
		pvalue = (float)(rand()%1000)/1000;
		if(pvalue>0.001)
		{
			Influence_star[i] = pvalue ;	
			i++ ;
	...
	while(i<N)
	{
	 ...
	 {	sum1 = 0.0 ;
		...
		{
			sum1 = sum1 + (float)Influence_star[i]*Susceptibility_star[Neighbors[i][j]] ;
			sum2 = sum2 + (float)Influence_star[Neighbors[i][j]]*Susceptibility_star[i] ;
		}
	 	Fstar[i] =  sum1 ;
	 	Gstar[i] =  sum2 ;
...
void Iterate_Influence_label(int Iteration, char **argv)
{
	...
	while(jj < Iteration)
	{  
		#pragma omp parallel for num_threads(24) //set the paralled threads
		for(i = 0 ; i < N ; i++)
		{
...
{	
	...
	for(ii = 0 ; ii < N ; ii++)
	{	
		for(jj = 0 ; jj < Degree[ii] ; jj++)
		{
			fprintf(fout,"%d %d %f\n",ii,Neighbors[ii][jj],Pij_weight[ii][jj]) ;
		}
	} 
	fclose(fout);	
}
void Output_detaildata(char **argv)
{
	char f1[27] = "results_theory/detaildata/";
	char f2[5] = ".txt" ;
	char *file_name = (char *) malloc((strlen(argv[1])+strlen(argv[2])+strlen(f1)+strlen(f2)+1)*sizeof(char)) ;
	sprintf(file_name,"%s%s_%s%s",f1,argv[1],argv[2],f2) ;
	int i  ;
	FILE *fout ;
	fout = fopen(file_name,"w") ;
	fprintf(fout,"%s %s %s %s %s %s\n","Inf_UN","Inf_GT","Sus_UN","Sus_GT","F_GT","G_GT") ;
	for(i = 0 ; i < N ; i++)
		fprintf(fout,"%.4f %.4f %.4f %.4f %.4f %.4f\n",Influence_star[i],Influence1[i],Susceptibility_star[i] , Susceptibility[i] , Fstar[i],Gstar[i]) ;
	fclose(fout)   ;	
}
void Generate_pij_weight()
{	
